...
var (
	...
	NuaFlag = &cli.BoolFlag{
		Name:     "hotdog",
		Usage:    "Nua network: pre-configured proof-of-authority test network",
		Category: flags.EthCategory,
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		...
		if ctx.Bool(NuaFlag.Name) {
			return filepath.Join(path, "nua")
		}
		return path
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(NuaFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "nua")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, RopstenFlag, RinkebyFlag, GoerliFlag, NuaFlag, SepoliaFlag, KilnFlag)
	...
	case ctx.Bool(NuaFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1155
		}
		cfg.Genesis = core.DefaultNuaGenesisBlock()
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(NuaFlag.Name):
		genesis = core.DefaultNuaGenesisBlock()
	case ctx.Bool(KilnFlag.Name):
