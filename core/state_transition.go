...
func (st *StateTransition) TransitionDb() (*ExecutionResult, error) {
	...
	if st.evm.Config.NoBaseFee && st.gasFeeCap.Sign() == 0 && st.gasTipCap.Sign() == 0 {
		...
		if ret, err := st.finalizeGasFee(effectiveTip); err != nil {
			st.state.RevertToSnapshot(snapshot)
			if _, err := st.finalizeGasFee(effectiveTip); err != nil {
...
func (st *StateTransition) finalizeGasFee(effectiveTip *big.Int) ([]byte, error) {
